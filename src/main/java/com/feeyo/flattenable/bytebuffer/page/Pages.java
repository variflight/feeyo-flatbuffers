...
public class Pages {
	...
	//
	protected ByteBufferPage[] allPageBuffers;
    //
	...
	public void initialize() {
    	allPageBuffers = new ByteBufferPage[pageCount];
    	for (int i = 0; i < pageCount; i++) 
          allPageBuffers[i] = new ByteBufferPage(ByteBuffer.allocateDirect(pageSize), chunkSize);
	}
	...
	public ByteBuffer allocate(int size) {
		...
		final int selectedPage = prevAllocatedPage.incrementAndGet() % allPageBuffers.length;
		//
		ByteBuffer byteBuf = allocateBuffer(theChunkCount, 0, selectedPage);
		if (byteBuf == null) 
			byteBuf = allocateBuffer(theChunkCount, selectedPage, allPageBuffers.length);
		//
		if ( byteBuf != null ) {
	...
	private ByteBuffer allocateBuffer(int theChunkCount, int startPage, int endPage) {
		for (int i = startPage; i < endPage; i++) {
			ByteBuffer buffer = allPageBuffers[i].allocateByteBuffer(theChunkCount);
			if (buffer != null) {
	...
	public boolean recycle(ByteBuffer theBuf) {
		...
		for (ByteBufferPage pageBuffer : allPageBuffers) {
